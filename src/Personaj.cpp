...
Personaj::Personaj(const Personaj &other)
    : nume_(other.nume_), nationalitate_(other.nationalitate_), nivel_(other.nivel_), dulap_(other.dulap_), sold_(other.sold_)
{
    ++totalPersonaje_;
}
...
{
    os << "Nume: " << nume_ << " | Nationalitate: " << nationalitate_
       << " | Nivel: " << nivel_ << " | Sold: " << sold_ << "\n";
    dulap_.Afiseaza(os);
}
...
void Personaj::Cumpara(std::unique_ptr<Articol> articol)
{
    ...
    {
        throw BudgetException("articolul \"" + articol->GetNume() + "\" costa " + std::to_string(articol->GetPret()) +
...
void Personaj::AfiseazaEchipamentDeSezon(const std::string &sezon, std::ostream &os) const
{
    ...
    {
        const auto *im = dynamic_cast<const Imbracaminte *>(a_ptr.get());
...
void Personaj::AfiseazaAnalizaInventar(const std::string &eveniment, std::ostream &os) const
{
    ...
    os << "  Articole potrivite pentru \"" << eveniment << "\": " << dulap_.NumarArticolePentruEveniment(eveniment) << "\n";
}
